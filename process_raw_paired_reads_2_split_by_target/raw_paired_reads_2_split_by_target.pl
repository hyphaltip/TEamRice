...
"error: $sample has $pairs. This sample should have 2 pairs or just 1.\n";
    ...
    #print OUTFILE "for i in `ls \$tmp_dir/split_by_target` ; do ~/bin/move_files.pl \$tmp_dir/split_by_target/\$i $current_dir $start_host; done\n";
    ...
    #print OUTFILE "if [ $start_host = `hostname` ]; then cd $current_dir; rm -rf \$tmp_dir; else cd ~ ; rm -rf \$tmp_dir; fi\n";
}
