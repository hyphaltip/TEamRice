...
`samtools index  $dir_path/$sample.$lowest_dir.sorted.bam`;
	...
	warn "$dir_path/$sample.$lowest_dir.sam does not exist, therefore cannot convert to bam file";
}
...
`samtools index  $dir_path/$sample.$lowest_dir.unPaired.sorted.bam`;
	...
	warn "$dir_path/$sample.$lowest_dir.unPaired.sam does not exist, therefore cannot convert to bam file";
}
